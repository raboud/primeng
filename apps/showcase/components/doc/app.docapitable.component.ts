...
@Component({
    ...
    template: ` @if (data) {
        ...
        @if (!data[0].data) {
            <div class="doc-tablewrapper mt-4">
                <table class="doc-table">
                    <thead>
                        <tr>
                            @for (key of getKeys(data[0]); track $index) {
                                <th [style]="{ 'min-width': key === 'variable' && '30rem' }">
                    ...
                    <tbody>
                        @for (prop of data; track prop) {
                            <tr>
                                @for (entry of getEntries(prop); track $index) {
                                    <td>
                                                    ...
                                                    @if (parameter.name) {
                                                        <div class="doc-option-params">
                                                            ...
                                                            @for (value of getType(parameter.type); track value; let i = $index) {
                                                                ...
                                                                @if (isLinkType(value)) {
                                                                    <a
                                                                        (click)="scrollToLinkedElement($event, value)"
                                                                        [ngClass]="{
        ...
        @if (data[0].data && data[0].data.length > 0) {
            @for (childData of data; track childData.id) {
