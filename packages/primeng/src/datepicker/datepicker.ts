        ...
        @if (!inline) {
            ...
            @if (showClear && !disabled() && value != null) {
                @if (!clearIconTemplate && !_clearIconTemplate) {
                    <TimesIcon [class]="cx('clearIcon')" (click)="clear()" />
                }
                @if (clearIconTemplate || _clearIconTemplate) {
                    <span [class]="cx('clearIcon')" (click)="clear()">
            ...
            @if (iconDisplay === 'input' && showIcon) {
                <span [class]="cx('inputIconContainer')">
                    @if (!inputIconTemplate && !_inputIconTemplate) {
                        <CalendarIcon (click)="onButtonClick($event)" [class]="cx('inputIcon')" />
                    }
            ...
            >
                ...
                @if (!timeOnly) {
                                            ...
                                            @for (week of month.dates; track week; let j = $index) {
                                                <tr>
                                                    ...
                                                    @for (date of week; track $index) {
                                                        <td [attr.aria-label]="date.day" [class]="cx('dayCell', { date })">
                    ...
                    @if (currentView === 'month') {
                        <div [class]="cx('monthView')">
                            @for (m of monthPickerValues(); track m; let i = $index) {
                                <span (click)="onMonthSelect($event, i)" (keydown)="onMonthCellKeydown($event, i)" [class]="cx('month', { month: m, index: i })" pRipple>
                                    {{ m }}
                    ...
                    @if (currentView === 'year') {
                        <div [class]="cx('yearView')">
                            @for (y of yearPickerValues(); track y) {
                                <span (click)="onYearSelect($event, y)" (keydown)="onYearCellKeydown($event, y)" [class]="cx('year', { year: y })" pRipple>
                                    {{ y }}
                                    @if (isYearSelected(y)) {
                                        <div class="p-hidden-accessible" aria-live="polite">
                                            {{ y }}
