...
@Component({
    ...
    template: `
        @if (loading && showLoader) {
            <div [class]="cx('mask')">
                @if (loadingIcon) {
                    <i [class]="cn(cx('loadingIcon'), loadingIcon)"></i>
                }
                @if (!loadingIcon) {
            ...
            <ng-template #buildInTable let-items let-scrollerOptions="options">
                <table #table role="table" [class]="cn(cx('table'), tableStyleClass)" [style]="tableStyle" [attr.id]="id + '-table'">
                    ...
                    @if (frozenValue || frozenBodyTemplate || _frozenBodyTemplate) {
                        <tbody role="rowgroup" [class]="cx('tbody')" [value]="frozenValue" [frozenRows]="true" [pTableBody]="scrollerOptions.columns" [pTableBodyTemplate]="frozenBodyTemplate || _frozenBodyTemplate" [frozen]="true"></tbody>
                    }
                    <tbody
                        role="rowgroup"
                    ...
                    @if (scrollerOptions.spacerStyle) {
                        <tbody role="rowgroup" [style]="'height: calc(' + scrollerOptions.spacerStyle.height + ' - ' + scrollerOptions.rows.length * scrollerOptions.itemSize + 'px);'" [class]="cx('virtualScrollerSpacer')"></tbody>
                    }
                    @if (footerGroupedTemplate || footerTemplate || _footerTemplate || _footerGroupedTemplate) {
                        <tfoot role="rowgroup" #tfoot [ngClass]="cx('footer')" [ngStyle]="sx('tfoot')">
                            <ng-container
                                *ngTemplateOutlet="
    ...
    template: `
        @if (!dt.expandedRowTemplate && !dt._expandedRowTemplate) {
            @for (rowData of value; track dt.rowTrackBy(rowIndex, rowData); let rowIndex = $index) {
                @if ((dt.groupHeaderTemplate || dt._groupHeaderTemplate) && !dt.virtualScroll && dt.rowGroupMode === 'subheader' && shouldRenderRowGroupHeader(value, rowData, getRowIndex(rowIndex))) {
                ...
                @if ((dt.groupFooterTemplate || dt._groupFooterTemplate) && !dt.virtualScroll && dt.rowGroupMode === 'subheader' && shouldRenderRowGroupFooter(value, rowData, getRowIndex(rowIndex))) {
                    <ng-container role="row">
                        <ng-container
                            *ngTemplateOutlet="
...
@Component({
    ...
    template: `
        @if (!(dt.sortIconTemplate || dt._sortIconTemplate)) {
            @if (sortOrder === 0) {
                <SortAltIcon [styleClass]="cx('sortableColumnIcon')" />
            }
            @if (sortOrder === 1) {
                <SortAmountUpAltIcon [styleClass]="cx('sortableColumnIcon')" />
            }
            @if (sortOrder === -1) {
                <SortAmountDownIcon [styleClass]="cx('sortableColumnIcon')" />
            }
        }
        @if (dt.sortIconTemplate || dt._sortIconTemplate) {
        ...
        @if (isMultiSorted()) {
            <span [class]="cx('sortableColumnBadge')">{{ getBadgeValue() }}</span>
        }
    `,
        ...
        <div [class]="cx('filter')">
            ...
            @if (showMenu && overlayVisible) {
                ...
                >
                    ...
                    @if (display === 'row') {
                        <ul [class]="cx('filterConstraintList')">
    ...
    template: `
        @if (filterTemplate) {
            ...
            @switch (type) {
                ...
                @case ('boolean') {
                    <p-checkbox [indeterminate]="filterConstraint?.value === null" [binary]="true" [ngModel]="filterConstraint?.value" (ngModelChange)="onModelChange($event)" />
                }
                @case ('date') {
                    <p-datepicker [ariaLabel]="ariaLabel" [placeholder]="placeholder" [ngModel]="filterConstraint?.value" (ngModelChange)="onModelChange($event)" appendTo="body"></p-datepicker>
                }
            }
        }
    `,
