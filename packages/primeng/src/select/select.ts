...
@Component({
    ...
    template: `
        ...
        >
            @if (checkmark) {
                @if (selected) {
                    <CheckIcon [styleClass]="cx('optionCheckIcon')" />
                }
                @if (!selected) {
                    <BlankIcon [styleClass]="cx('optionBlankIcon')" />
                }
            }
            @if (!template) {
    ...
    template: `
        ...
        @if (isVisibleClearIcon) {
            @if (!clearIconTemplate && !_clearIconTemplate) {
                <TimesIcon [class]="cx('clearIcon')" (click)="clear($event)" [attr.data-pc-section]="'clearicon'" />
            }
            @if (clearIconTemplate || _clearIconTemplate) {
                <span [class]="cx('clearIcon')" (click)="clear($event)" [attr.data-pc-section]="'clearicon'">
        ...
        <div [class]="cx('dropdown')" role="button" aria-label="dropdown trigger" aria-haspopup="listbox" [attr.aria-expanded]="overlayVisible ?? false" [attr.data-pc-section]="'trigger'">
            @if (loading) {
                ...
                @if (!dropdownIconTemplate && !_dropdownIconTemplate) {
                    @if (dropdownIcon) {
                        <span [class]="cn(cx('dropdownIcon'), dropdownIcon)"></span>
                    }
                    @if (!dropdownIcon) {
